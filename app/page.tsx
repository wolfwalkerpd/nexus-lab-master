    ...
    <>
      ...
      <section className="wrap pb-section pt-5">
        ...
        <div className="grid grid-cols-1 gap-[22px] md:grid-cols-2">
          {WORK.map((w) => (
            <Link key={w.slug} href={`/work/${w.slug}`} className="block overflow-hidden rounded-[13px] border text-ink no-underline hair">
          ...
          <Link href="/work" className="flex min-h-[120px] flex-col items-start justify-center gap-[10px] rounded-[13px] border border-dashed bg-panel p-6 text-ink no-underline sm:p-7" style={{ borderColor: "rgba(var(--ink-rgb),0.28)" }}>
      ...
      <section className="border-t bg-panel hair">
        <div className="wrap py-section">
          ...
          <div className="grid grid-cols-1 items-stretch gap-5 sm:grid-cols-2 md:grid-cols-3">
            {BUILD_PLANS.map((p) => (
              <div key={p.name} className="flex flex-col rounded-[15px] border bg-card p-6 sm:p-8" style={{ borderColor: p.popular ? "var(--accent)" : "rgba(var(--ink-rgb),0.14)" }}>
